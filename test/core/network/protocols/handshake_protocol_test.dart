...
void main() {
  ...
  group('HandshakeProtocol Tests', () {
    test('createHandshakeMessage should generate valid JSON with Bloom Filter', () async {
      // Arrange
      when(() => mockAuthService.currentUser).thenReturn(
        UserModel(userId: 'user123', phoneNumber: '+1234567890'),
      );
      when(() => mockKeyManager.getPublicKey()).thenAnswer((_) async => [1, 2, 3, 4]);
      when(() => mockDatabase.getAllKnownMessageIds()).thenAnswer((_) async => ['msg1', 'msg2']);

      final protocol = HandshakeProtocol(container.read);
    ...
    test('processIncomingHandshake should accept known contact', () async {
      ...
      when(() => mockDatabase.getContactById('peer123')).thenAnswer((_) async => contact);
      when(() => mockDatabase.updateContact(any(), any())).thenAnswer((_) async => true);
      when(() => mockAuthService.currentUser).thenReturn(
        UserModel(userId: 'user123', phoneNumber: '+1234567890'),
      );
      when(() => mockDatabase.getAllKnownMessageIds()).thenAnswer((_) async => ['msg1']);
    ...
    test('processIncomingHandshake should reject unknown contact', () async {
      // Arrange
      when(() => mockDatabase.getContactById('unknown123')).thenAnswer((_) async => null);
      when(() => mockAuthService.currentUser).thenReturn(
        UserModel(userId: 'user123', phoneNumber: '+1234567890'),
      );
    ...
    test('processHandshakeAck should parse accepted ACK with Bloom Filter', () async {
      // Arrange
      final protocol = HandshakeProtocol(container.read);
    ...
    test('processIncomingHandshake should handle missing Bloom Filter gracefully', () async {
      // Arrange
      final contact = MockContactsTableData();
      ...
      when(() => mockDatabase.getAllKnownMessageIds()).thenAnswer((_) async => []);
