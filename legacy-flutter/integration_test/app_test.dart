...
void main() {
  ...
  group('Sada App Integration Test - Happy Path', () {
    testWidgets('Complete user journey from launch to settings', (
      ...
      LogService.info('✅ التطبيق تم إطلاقه');
      ...
      if (nameFieldCheck.evaluate().isNotEmpty || 
        ...
        LogService.info('📝 تم العثور على Register Screen مباشرة - إدخال الاسم...');
        ...
        // ============================================
        LogService.info('📱 تم العثور على Lock Screen - إدخال PIN...');
        ...
        LogService.info('✅ تم إدخال PIN بنجاح');
        ...
        // ============================================
        LogService.info('📱 تم العثور على Onboarding Screen...');
        ...
        LogService.info('   🔍 البحث عن Register Screen...');
        LogService.info('   - عنوان عربي موجود: ${registerTitleArCheck2.evaluate().isNotEmpty}');
        LogService.info('   - عنوان إنجليزي موجود: ${registerTitleEnCheck2.evaluate().isNotEmpty}');
        LogService.info('   - حقل الإدخال موجود: ${nameFieldCheck2.evaluate().isNotEmpty}');
        
        if (registerTitleArCheck2.evaluate().isNotEmpty || 
            registerTitleEnCheck2.evaluate().isNotEmpty ||
            nameFieldCheck2.evaluate().isNotEmpty) {
          LogService.info('📝 تم العثور على Register Screen - إدخال الاسم...');
          ...
          if (nameField.evaluate().isNotEmpty) {
            LogService.info('   📍 تم العثور على حقل الاسم بالـ Key');
            ...
            LogService.info('   📝 النص المدخل في الحقل: ${textField.controller?.text ?? "غير متاح"}');
            LogService.info('   ✅ تم إدخال النص: TestUser');
          } else {
            // محاولة البحث بالـ TextFormField
            final nameFieldByType = find.byType(TextFormField);
            if (nameFieldByType.evaluate().isNotEmpty) {
              LogService.info('   📍 تم العثور على حقل الاسم بالـ Type');
              ...
              LogService.info('   ⚠️ لم يتم العثور على حقل الإدخال');
            }
          }
          ...
          if (registerButton.evaluate().isNotEmpty) {
            LogService.info('   📍 تم العثور على زر التسجيل بالـ Key');
          ...
          if (pinSetupTitle.evaluate().isNotEmpty || pinSetupTitleAr.evaluate().isNotEmpty) {
            LogService.info('   📱 تم العثور على شاشة إعداد PIN - تخطي...');
            await tester.pumpAndSettle(const Duration(seconds: 2));
          }
          
          LogService.info('✅ تم التسجيل بنجاح');
          ...
          if (skipButtonAr2.evaluate().isNotEmpty || 
            ...
            LogService.info('📱 تم العثور على Onboarding Screen بعد التسجيل - إكمال Onboarding...');
            
            // محاولة الضغط على Skip أولاً (أسرع)
            if (skipButtonAr2.evaluate().isNotEmpty) {
              LogService.info('   📍 الضغط على زر "تخطي" (عربي)');
              await tester.tap(skipButtonAr2);
              await tester.pumpAndSettle(const Duration(seconds: 3));
            } else if (skipButtonEn2.evaluate().isNotEmpty) {
              LogService.info('   📍 الضغط على زر "Skip" (إنجليزي)');
              await tester.tap(skipButtonEn2);
              await tester.pumpAndSettle(const Duration(seconds: 3));
            } else {
              // إذا لم يكن Skip موجوداً، نمرر عبر Slides
              LogService.info('   📍 التمرير عبر Onboarding slides...');
              ...
              if (getStartedButtonAr2.evaluate().isNotEmpty) {
                LogService.info('   📍 الضغط على زر "ابدأ" (عربي)');
                await tester.tap(getStartedButtonAr2);
                await tester.pumpAndSettle(const Duration(seconds: 3));
              } else if (getStartedButtonEn2.evaluate().isNotEmpty) {
                LogService.info('   📍 الضغط على زر "Get Started" (إنجليزي)');
        ...
        if (nameFieldDirect.evaluate().isNotEmpty || 
          ...
          if (nameFieldDirect.evaluate().isNotEmpty) {
            ...
            try {
              ...
              LogService.info('   📝 النص المدخل في الحقل: "$enteredText"');
              
              if (enteredText.isEmpty || enteredText != 'TestUser') {
                // محاولة إدخال النص مرة أخرى
                LogService.info('   ⚠️ النص لم يتم إدخاله بشكل صحيح، المحاولة مرة أخرى...');
                await tester.tap(nameFieldDirect);
                await tester.pump(const Duration(milliseconds: 300));
                await tester.enterText(nameFieldDirect, 'TestUser');
                await tester.pump(const Duration(milliseconds: 1000));
              ...
              LogService.info('   ⚠️ لا يمكن الوصول إلى controller: $e');
            }
          ...
          if (registerButton.evaluate().isNotEmpty) {
            ...
            try {
              ...
              if (buttonWidget.onPressed == null) {
                LogService.info('   ⚠️ زر التسجيل معطل - انتظار قليل...');
                await tester.pumpAndSettle(const Duration(seconds: 2));
              }
            } catch (e) {
              LogService.info('   ⚠️ لا يمكن التحقق من حالة الزر: $e');
            }
      ...
      // ============================================
      LogService.info('🏠 التحقق من Home Screen...');
      ...
      if (!homeFound) {
        LogService.info('   ⚠️ لم يتم العثور على Home Screen - محاولة الانتظار أكثر...');
        await tester.pumpAndSettle(const Duration(seconds: 5));
      ...
      LogService.info('✅ تم التحقق من Home Screen');

      // التحقق من وجود FAB (Radar button) - تم التحقق منه بالفعل أعلاه
      if (fab.evaluate().isEmpty && fabByIcon.evaluate().isEmpty) {
        LogService.info('   ⚠️ FAB غير موجود - لكن هذا لا يمنع نجاح الاختبار');
      } else {
        LogService.info('✅ تم التحقق من FAB (Radar button)');
      }

      // ============================================
      // Step 4: Navigate to Settings
      // ============================================
      LogService.info('⚙️ الانتقال إلى Settings...');
      ...
      if (settingsNavButton.evaluate().isNotEmpty) {
        LogService.info('   📍 تم العثور على Settings button بالـ Key');
        ...
        if (settingsTooltip.evaluate().isNotEmpty) {
          LogService.info('   📍 تم العثور على Settings button بالـ Tooltip (عربي)');
          ...
          if (settingsIcons.evaluate().isNotEmpty) {
            LogService.info('   📍 تم العثور على Settings button بالـ Icon (${settingsIcons.evaluate().length} أيقونة)');
            ...
            if (settingsTextAr.evaluate().isNotEmpty) {
              LogService.info('   📍 تم العثور على Settings button بالنص (عربي)');
              await tester.tap(settingsTextAr);
            } else if (settingsTextEn.evaluate().isNotEmpty) {
              LogService.info('   📍 تم العثور على Settings button بالنص (إنجليزي)');
              ...
              if (navBar.evaluate().isNotEmpty) {
                LogService.info('   📍 محاولة النقر على Settings في NavigationBar (آخر destination)');
                ...
                LogService.info('   ⚠️ لم يتم العثور على Settings button - محاولة النقر على موضع تقريبي');
      ...
      LogService.info('✅ تم فتح Settings Screen');

      // ============================================
      // Step 5: Interact with Settings
      // ============================================
      LogService.info('🔧 التفاعل مع Settings...');
      ...
      if (themeTileAr.evaluate().isNotEmpty) {
        ...
        await tester.pumpAndSettle();
        LogService.info('✅ تم النقر على Theme (عربي)');
      } else if (themeTileEn.evaluate().isNotEmpty) {
        await tester.tap(themeTileEn);
        await tester.pumpAndSettle();
        LogService.info('✅ تم النقر على Theme (إنجليزي)');
      }
      ...
      LogService.info('✅ تم التحقق من وجود Power Mode');

      // ============================================
      // Step 6: Navigate Back to Home
      // ============================================
      LogService.info('🏠 العودة إلى Home...');
      ...
      if (homeNavButton.evaluate().isNotEmpty) {
        LogService.info('   📍 تم العثور على Home button بالـ Key');
        ...
        if (homeTooltip.evaluate().isNotEmpty) {
          ...
          if (homeIcons.evaluate().isNotEmpty) {
            ...
            if (homeTextAr.evaluate().isNotEmpty) {
              LogService.info('   📍 تم العثور على Home button بالنص (عربي)');
              await tester.tap(homeTextAr);
            } else if (homeTextEn.evaluate().isNotEmpty) {
              LogService.info('   📍 تم العثور على Home button بالنص (إنجليزي)');
      ...
      LogService.info('✅ تم العودة إلى Home Screen بنجاح');
