...
|--------|------------------------|---------------------------|----------------------------|----------|--------|
| P0-ACK-1 | Finalize end-to-end ACK pipeline so that every final recipient emits a MeshMessage/RelayPacket ACK back to the original sender, updating MessageStatus to `delivered` only when ACK arrives. | S-003, S-004, S-012, S-017, S-021, S-022, S-026, S-081 | `lib/core/network/mesh_service.dart` (`sendMeshMessage`, `handleIncomingMeshMessage`, `_handleAck`), `lib/core/network/incoming_message_handler.dart`, `lib/core/network/models/mesh_message.dart`, `lib/core/database/app_database.dart` (`updateMessageStatus`), `lib/features/chat/application/chat_controller.dart` | P0 | TODO |
| P0-ACK-2 | Ensure ACK packets themselves follow DTN semantics (RelayPacket-based store-carry-forward, TTL, dedup) so that ACKs can traverse multiple hops reliably. | S-003, S-004, S-012, S-017, S-022 | `lib/core/network/router/epidemic_router.dart` (handling of ACK in summaries/requests), `lib/core/network/models/relay_packet.dart`, `AppDatabase` RelayQueue methods | P0 | TODO |
| P0-ACK-3 | Align `MessageStatus` transitions with ACK logic (`draft → sending → sent → delivered → read/failed`) and remove any fake or time-based auto-`delivered` heuristics. | S-041, S-042, S-048 | `lib/features/chat/domain/models/message_model.dart`, `lib/features/chat/data/mappers/message_mapper.dart`, `lib/features/chat/presentation/widgets/message_bubble.dart`, `lib/core/database/app_database.dart` | P0 | TODO |
...
| Task ID | Description (English) | Linked Scenarios (S-XXX) | Code Areas (files/modules) | Priority | Status |
|--------|------------------------|---------------------------|----------------------------|----------|--------|
| P0-CON-1 | Validate and tune per-peer Token Bucket settings (tokens per minute, refill interval) in `EpidemicRouter` under simulated high-density conditions; derive recommended defaults for pilots. | S-022, S-025, S-028, S-029, S-030 | `lib/core/network/router/epidemic_router.dart` (`_peerTokens`, `_maxTokensPerPeer`, `_tokenRefillInterval`, `_canSendToPeer`), new tests in `test/` simulating flooding | P0 | TODO |
| P0-CON-2 | Extend RelayQueue quota from simple count-based limit to approximate byte-based quota (e.g., 50–100 MB), with clear eviction policy (oldest, failed, or low-priority packets first). | S-004, S-012, S-021, S-022, S-025, S-026 | `lib/core/database/app_database.dart` (`enqueueRelayPacket`, `_trimRelayQueue`, `getRelayStorageSize`), `lib/core/utils/constants.dart` (size thresholds) | P0 | TODO |
| P0-CON-3 | Implement optional per-message or per-category priority flag (e.g., panic vs normal) in `RelayPacket`/`MeshMessage`, to bias eviction and congestion decisions for future panic/SOS use. | S-025, S-097 | `lib/core/network/models/relay_packet.dart`, `mesh_message.dart`, `AppDatabase` schema & migrations | P0 | TODO |
...
| P0-CON-5 | Add runtime metrics (visible in debug screen) for RelayQueue size, dropped packets due to quota, and token-bucket drops per peer. | S-022, S-028, S-081 | `lib/features/mesh/presentation/mesh_debug_screen.dart` (or new), `epidemic_router.dart`, `app_database.dart` | P0 | TODO |
...
| Task ID | Description (English) | Linked Scenarios (S-XXX) | Code Areas (files/modules) | Priority | Status |
|--------|------------------------|---------------------------|----------------------------|----------|--------|
| P0-BG-1 | Review and harden `BackgroundService` lifecycle: ensure foreground notification is always present when DTN is active, handle `onStart`/`stop` flows robustly, and avoid silent service death. | S-051, S-054, S-055, S-063, S-069 | `lib/core/services/background_service.dart`, Android manifest & notification channel config | P0 | TODO |
...
| P0-BG-3 | Implement practical PowerMode/duty cycle policies (balanced/high/low) backed by real scan/sleep intervals, and ensure EpidemicRouter’s `startDutyCycle`/`stopDutyCycle` are consistent with BackgroundService. | S-051, S-052, S-055, S-056, S-058 | `background_service.dart`, `lib/core/models/power_mode.dart`, `lib/core/power/discovery_strategy.dart`, `epidemic_router.dart` | P0 | TODO |
...
| Task ID | Description (English) | Linked Scenarios (S-XXX) | Code Areas (files/modules) | Priority | Status |
|--------|------------------------|---------------------------|----------------------------|----------|--------|
| P0-UX-1 | Add concise in-app explanations (onboarding + help screens) for DTN behavior: delays, multi-hop, offline-first; ensure users do not expect internet-like real-time delivery. | S-021, S-041, S-042, S-044, S-048, S-049 | `features/onboarding/*`, `features/settings/help_about_screen.dart` (or new), localized strings | P0 | TODO |
...
| P0-UX-4 | Improve duress UX: ensure flows for setting/using duress PIN are clear to the owner but do not reveal duress mode to an observer; document behavior in a privacy-respecting way. | S-018, S-031, S-032, S-040, S-045 | `auth_service.dart`, `features/auth/presentation/*`, `features/settings/duress_settings_screen.dart` (if present/added) | P0 | TODO |
...
| P0-FAIL-2 | Add robust JSON parsing and quarantine for malformed RelayQueue payloads (invalid MeshMessage JSON); ensure they do not crash the router or message handler. | S-072, S-078 | `incoming_message_handler.dart`, `mesh_service.dart`, `epidemic_router.dart`, tests in `test/relayqueue_malformed_test.dart` | P0 | TODO |
...
| Task ID | Description (English) | Linked Scenarios (S-XXX) | Code Areas | Priority | Status |
|--------|------------------------|---------------------------|-----------|----------|--------|
| P1-OBS-1 | Implement a `Mesh Debug Screen` accessible from settings (developer/advanced section) showing: peer count, RelayQueue size, PowerMode, ACK stats, and recent errors. | S-051, S-055, S-081, S-082 | `features/mesh/presentation/mesh_debug_screen.dart` (new), hooks from `background_service.dart`, `epidemic_router.dart`, `app_database.dart` | P1 | TODO |
| P1-OBS-2 | Add simple log export mechanism (redacted) so pilot operators can export logs after a run for offline analysis. | S-081–S-088 | `log_service.dart`, `features/settings/log_export_screen.dart` | P1 | TODO |
...
| P1-TEST-2 | Add tests around `HandshakeProtocol` and contact whitelisting to ensure unknown/blocked senders are never decrypted or shown. | S-031, S-032, S-038, S-039, S-100 | `test/handshake_whitelist_test.dart`, `incoming_message_handler.dart`, `handshake_protocol.dart` | P1 | TODO |
